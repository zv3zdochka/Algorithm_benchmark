...
def save_rows(rows: List[Dict[str, object]], out_csv: str):
    if not rows:
        return
    keys = ["algo","n","mode","seed","time_limit","status","cost","opt_cost","rel_error","elapsed","iterations"]
...
def run_once(args, n: int, C: Optional[np.ndarray]) -> List[Dict[str, object]]:
    ...
    if args.algo in ("aco","all"):
        res = run_aco(C, args); add_row(res)
        if args.outdir:
            plot_convergence(res.get("history",[]), os.path.join(args.outdir, f"convergence_aco_n{n}.png"),
                             f"ACO convergence (n={n})")
            if args.history_csv:
                # сохранить историю отдельно, если просили
                pass
    if args.algo in ("hungarian_manual","all"):
        res = run_hungarian_manual(C, args); add_row(res)
    if args.algo in ("hungarian_scipy","all"):
        res = run_hungarian_scipy(C, args); add_row(res)
    if args.algo in ("greedy","all"):
        res = run_greedy(C, args); add_row(res)
...
def main():
    ...
    for n in sizes:
        ...
        plot_bars = None
        try:
            from matplotlib import pyplot as plt  # check availability
            def _plot_bars(rows, out_png, metric, title):
                import matplotlib.pyplot as plt
                algos = [r["algo"] for r in rows]
                vals = [float(r.get(metric,0) or 0) if r.get(metric,"")!="" else 0.0 for r in rows]
                plt.figure(); plt.bar(algos, vals); plt.ylabel(metric); plt.title(title); plt.tight_layout(); plt.savefig(out_png); plt.close()
            plot_bars = _plot_bars
        except Exception:
            pass
        if plot_bars:
            plot_bars(rows, per_n_png_elapsed, "elapsed", f"Elapsed time (n={n})")
            plot_bars(rows, os.path.join(args.outdir, f"bars_rel_error_n{n}.png"), "rel_error",
                      f"Relative error (n={n})")
            ...
            def _plot_lines(metric, out_png, title):
                ...
                for a in algos:
                    ...
                    for r in dat:
                        ...
                        v = r.get(metric,"")
                        if v=="": continue
                        xs.append(int(r["n"])); ys.append(float(v))
                    if xs:
                        pairs = sorted(zip(xs,ys)); xs=[p[0] for p in pairs]; ys=[p[1] for p in pairs]
                        plt.plot(xs, ys, label=a)
                plt.xlabel("n"); plt.ylabel(metric); plt.title(title); plt.legend(); plt.tight_layout(); plt.savefig(out_png); plt.close()
            _plot_lines("elapsed", os.path.join(args.outdir, "lines_elapsed_vs_n.png"), "Elapsed vs n")
            _plot_lines("rel_error", os.path.join(args.outdir, "lines_rel_error_vs_n.png"), "Relative error vs n")
        except Exception:
            pass
